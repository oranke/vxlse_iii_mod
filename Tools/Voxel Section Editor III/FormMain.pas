...
const
  ...
  APPLICATION_VER = '1.38 (mod by oranke)';
...
begin
  if not isEditable then
    Exit;
  if not scrollbar_editable then
    Exit;
...
begin
  ...
  begin
    if l = 0 then
    begin
      frm.Loading.Caption:= 'Loading: 3rd Party Palettes';
      frm.Loading.Refresh;
      LoadPalettes;
    end;
    if l = 10 then
    begin
      frm.Loading.Caption:= 'Loading: Colour Schemes';
      frm.Loading.Refresh;
      LoadCScheme;
    end;
    if l = 23 then
    begin
      frm.Loading.Caption:= 'Finished Loading';
      delay(50);
    end;
    l:=l + 1;
    delay(2);
...
begin
  TranslateClick2(0, X, Y, LastClick[0].X, LastClick[0].Y, LastClick[0].Z);
  if (LastClick[0].X < 0) or (LastClick[0].Y < 0) or (LastClick[0].Z < 0) then
    Exit;

  with ActiveSection.Tailer do
    if (LastClick[0].X > XSize - 1) or (LastClick[0].Y > YSize - 1) or (LastClick[0].Z > ZSize - 1) then
      Exit;
...
{$ENDIF}
  ...
  else if @Application.OnIdle = nil then
    Application.OnIdle:=Idle;
end;
