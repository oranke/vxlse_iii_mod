...
begin
  ...
  // set views
  if Assigned(View[0]) then
    FreeAndNil(View[0]);
  if Assigned(View[1]) then
    FreeAndNil(View[1]);
  if Assigned(View[2]) then
    FreeAndNil(View[2]);
...
begin
  if not Loaded then
    Exit; // cannot save an empty voxel?
...
begin
  ...
  if (Zpos = Zpos2) then
    O:=oriZ;
  if (O = oriUnDef) then
    MessageDlg('Impossible to draw 3D rectangles!!!', mtError, [mbOK], 0);
...
type
  ...
  begin
    PointOK:=False;
    if (l.X < 0) or (l.Y < 0) or (l.Z < 0) then
      Exit;
    if (l.X >= Tailer.XSize) or (l.Y >= Tailer.YSize) or (l.Z >= Tailer.ZSize) then
      Exit;
    PointOK:=True;
  end;
var
