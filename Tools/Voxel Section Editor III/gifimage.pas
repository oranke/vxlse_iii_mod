...
begin
  ...
  else if (Source is TGIFColorMap) then
  begin
    Clear;
...
begin
  ...
  begin
    ...
    {// TGIFSubImage import} if (Source is TGIFSubImage) then
    begin
      ...
      {// Bitmap import} if (Source is TBitmap) then
      begin
        // Zap existing colormap, extensions and bitmap
        Clear;
...
{$ENDIF}
        ...
        {// TGraphic import} if (Source is TGraphic) then
        begin
          // Zap existing colormap, extensions and bitmap
          Clear;
          ...
          {// TPicture import} if (Source is TPicture) then
          begin
...
begin
  ...
  else if (Size <> sizeof(TGIFApplicationRec)) or
...
{$ENDIF}
              ...
              else if (Ext is TGIFGraphicControlExtension) then
...
begin
  ...
  else if (Source is TGIFSubImage) then
...
begin
  ...
  try
    ...
    else if (FHeader.ColorMap.Count > 0) then
    begin
      // or create a new if first time
...
begin
  ...
  begin
    ...
    {// TGIFImage import} if (Source is TGIFImage) then
    begin
      Clear;
...
begin
  ...
  try
    ...
    else if (FBitmap <> nil) and not (goDirectDraw in FDrawOptions) then
    begin
